...
// ---------------------------------------------------------------------------
// Safe runtime messaging — guards every chrome.runtime.sendMessage call.
//
// "Extension context invalidated" is thrown when the extension is reloaded or
// updated while a tab is still open.  The old content script keeps running with
// its DOM event listeners intact, but chrome.runtime.id becomes undefined and
// any chrome.runtime API call throws.  Checking the id first AND wrapping in
// try/catch stops Chrome from logging uncaught errors in the extension console.
//
// onError (optional) is called when the background is unreachable — used by
// the timer-relay sleep() so it can fall back to a plain setTimeout.
// ---------------------------------------------------------------------------
function sendRuntimeMessage(msg, onError) {
  try {
    if (!chrome.runtime?.id) {
      if (onError) onError();
      return Promise.resolve();
    }
    return chrome.runtime.sendMessage(msg).catch(() => { if (onError) onError(); });
  } catch (_) {
    if (onError) onError();
    return Promise.resolve();
  }
...
// Cross-frame mousedown tracking is only needed while a typing session is
// active somewhere in this tab.  The background enables/disables it via
// "enable-mousedown-tracking" / "disable-mousedown-tracking" messages so we
// never send frame-mousedown while idle — preventing spurious service worker
// wake-ups in Chrome MV3 that caused extension console errors.
let _trackMousedowns = false;
...
function doPause() {
  if (!stopTyping && !pausedTyping) {
    pausedTyping = true;
    sendRuntimeMessage({ action: "auto-paused" });
  }
}
...
// Cross-frame mousedown detection — when a typing session is active, each frame
// reports mousedowns to the background.  If a click lands in a frame other than
// the one being typed into (e.g. user clicks the Slides toolbar while the canvas
// iframe is typing) the background relays an "external-mousedown" to trigger
// auto-pause.  _trackMousedowns is enabled only during an active session so we
// never wake up the Chrome MV3 service worker on idle page clicks.
document.addEventListener("mousedown", () => {
  if (_trackMousedowns) {
    sendRuntimeMessage({ action: "frame-mousedown" });
  }
}, true);
...
// Click fallback — some complex editors (e.g. Google Docs' about:blank typing
// iframe) don't reliably fire focusin on the element that receives keyboard
// input.  On any click we check document.activeElement and body.isContentEditable
// as a secondary detection path.
document.addEventListener("click", () => {
  // Wait a tick for the browser to settle focus after the click
  setTimeout(() => {
    const active = document.activeElement;
    // If the whole document body is the editable surface (Google Docs iframe),
    // treat the body itself as the target.
    const candidate =
      (active && (isTypable(active) || active.isContentEditable))
        ? active
        : (document.body?.isContentEditable ? document.body : null);
    if (candidate && candidate !== lastFocusedEl) {
...
window.addEventListener("message", (event) => {
  if (!_overlayIframe || event.source !== _overlayIframe.contentWindow) return;
  if (event.data?.source !== "text-to-write-overlay") return;

  if (event.data.action === "close-overlay") {
    closeOverlay();
  }
});
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  // Background enables/disables cross-frame mousedown reporting
  if (message.action === "enable-mousedown-tracking") {
    _trackMousedowns = true;
    return false;
  }
  if (message.action === "disable-mousedown-tracking") {
    _trackMousedowns = false;
    return false;
  }
  ...
  // Reject elements that have been removed from the DOM since they were focused.
  // This can happen when a message is broadcast to all frames (e.g., after the
  // background service worker restarted and lost its frame-tracking state) and a
  // stale lastFocusedEl in this frame points to an element that no longer exists.
  ...
  el.focus();
  attachBlurPause(el);
  sendRuntimeMessage({ action: "typing-started" });
  isTypingActive = true;
  ...
  typeText(el, message.text, message.delay, opts)
    .then((stopped) => {
      ...
      detachBlurPause();
      sendRuntimeMessage({ action: "typing-stopped" });
      sendResponse({ success: true, stopped });
      sendRuntimeMessage({ action: "typing-complete", stopped });
    })
    .catch((err) => {
      isTypingActive = false;
      detachBlurPause();
      ...
      sendRuntimeMessage({ action: "typing-complete", stopped: true, error: err.message });
    });
  ...
  for (const token of tokens) {
    ...
    if (doMistake) {
      ...
      if (_now1 - _lastProgressSent >= 300) {
        ...
        sendRuntimeMessage({ action: "typing-progress", wordsTyped });
      }
...
// ---------------------------------------------------------------------------
// Emoji insertion — cascading strategies, cleanest first
// ---------------------------------------------------------------------------
function emojiInsert(el, char) {
  ...
  // Works in Chrome for contentEditable emoji insertion.
  // The emoji is valid HTML text content so no escaping is needed.
...
function sleep(ms) {
  return new Promise((resolve) => {
    ...
    sendRuntimeMessage({ action: "timer-request", id, ms }, () => {
      // Background unreachable (or context invalidated) — fall back to setTimeout
      _timers.delete(id);
...
function openOverlay() {
  if (_overlayHost || !document.documentElement) return;

  _overlayHost = document.createElement("div");
  _overlayHost.id = "text-to-write-overlay-host";

  const shadow = _overlayHost.attachShadow({ mode: "closed" });
  ...
  _overlayIframe.src = chrome.runtime.getURL("popup.html?mode=overlay");
  _overlayIframe.setAttribute("title", "Text to Write");

  wrap.appendChild(_overlayIframe);
